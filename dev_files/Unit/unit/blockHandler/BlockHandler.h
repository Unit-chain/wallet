...
void BlockHandler::startBlockGenerator() {
    ...
    while (true) {
        ...
        std::shared_ptr<rocksdb::WriteBatch> txBatch = unit::BasicDB::getBatch();  // аккумулятор изменений
        ...
        if (txBatch->HasPut()) this->transactionWriter.commit(txBatch); // использовать
